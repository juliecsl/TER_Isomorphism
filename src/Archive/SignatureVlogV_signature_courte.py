...
def SignaturePartitionnement(graph: list) -> dict:
    ...
    FirstPartitioningDico = FirstPartitioning(graph) # Dictionnaire représentant les blocs de partionnement
    # Tri du dictionnaire dans l'ordre croissant
    # Pour que l'algo s'execute tjr dans le meme ordre de partitions
    FirstPartitioningDico = dict(sorted(FirstPartitioningDico.items()))

    table_hachage = dict()
    blocks = dict()
    ind = 0
    for key, value in FirstPartitioningDico.items():
        table_hachage[ind] = key
        blocks[ind] = value 
        ind += 1
...
# s1, table = SignaturePartitionnement(graph1)
