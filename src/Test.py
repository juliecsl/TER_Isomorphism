...
class TestUtils(unittest.TestCase):
    ...
    def test1_create_isomorphism(self):
        ...
        self.assertEqual(ReadGraphFromWeb(self.filename2), res)
...
class TestVersionNaive(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.graph1 = Graph(ReadGraphFromWeb(filename1))

        filename1bis = "FichierTests/graph1PASISO.txt"
        self.graph1bis = Graph(ReadGraphFromWeb(filename1bis))
        
        filename2 = "FichierTests/graph2.txt"
        self.graph2 = Graph(ReadGraphFromWeb(filename2))

        filename2bis = "FichierTests/graph2bis.txt"
        self.graph2bis = Graph(ReadGraphFromWeb(filename2bis))


    def test1_isomorphisme(self):
        
        self.assertEqual(Isomorphisme(self.graph2, self.graph2bis), True,
                         'graphes isomorphes')
        
    def test2_isomorphisme(self):
        
        self.assertEqual(Isomorphisme(self.graph1, self.graph1bis), False, 'graphes non isomorphes')
